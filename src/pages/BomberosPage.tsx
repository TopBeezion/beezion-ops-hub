...
export function BomberosPage() {
  ...
  const [visibleCols, setVisibleCols] = useState<Record<TaskStatus, boolean>>({
    pendiente: true,
    en_proceso: true,
    aprobacion_interna: true,
    correcciones: true,
    enviado_cliente: true,
    ajustes_cliente: true,
    blocker: true,
    done: false,
  })
  ...
  const byStatus = useMemo(() => {
    const map: Record<TaskStatus, Task[]> = {
      pendiente: [], en_proceso: [], aprobacion_interna: [], correcciones: [],
      enviado_cliente: [], ajustes_cliente: [], blocker: [], done: [],
    }
  ...
  const stats = useMemo(() => ({
    ...
    enProceso:  tasks.filter(t => t.priority === 'alta' && t.status === 'en_proceso').length,
    pendientes: tasks.filter(t => t.priority === 'alta' && t.status === 'pendiente').length,
    blocker:    tasks.filter(t => t.priority === 'alta' && t.status === 'aprobacion_interna').length,
    done:       tasks.filter(t => t.priority === 'alta' && t.status === 'done').length,
  }), [tasks])
